...
const PendingCollectors = () => {
  ...
  // const apiUrl1="http://localhost:1200/api/v1/admin/collector/getAllPendingCollectors"
  const apiUrl1 =
    "https://famous-foal-khakis.cyclic.app/api/v1/admin/collector/getAllPendingCollectors";
  //const apiUrl2=`http://localhost:1200/api/v1/collector/delete/${collectorId}`
  useEffect(() => {
    axios
      .get(apiUrl1)
      .then((response) => {
        setPendingCollectors(response.data.pendingCollectors);
        setIsLoading(false)
      })
      .catch((error) => {
        setError(error);
        ...
        <div className="w-full ">
          <div className="max-w-screen-lg w-full overflow-hidden shadow-md">
            <table className="w-full table-auto">
              ...
              <tbody>
                {pendingCollectors.map((collector, index) => (
                  <tr
                    key={collector._id}
