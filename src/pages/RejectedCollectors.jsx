...
const RejectedCollectors = () => {
  const [rejectedCollectors, setRejectedCollectors] = useState([]);
  const [error, setError] = useState("");
  const [isLoading, setIsLoading] = useState(true);
  //const apiUrl1="http://localhost:1200/api/v1/admin/collector/getAllRejectedCollectors"
  const apiUrl1 =
    "https://famous-foal-khakis.cyclic.app/api/v1/admin/collector/getAllRejectedCollectors";

  useEffect(() => {
    axios
      .get(apiUrl1)
      .then((response) => {
        setRejectedCollectors(response.data.rejectedCollectors);
        if(rejectedCollectors.length==0){
          <h1>No Rejected collector</h1>
          setIsLoading(false);
        }
        ...
        <div className="w-full ">
          <div className="max-w-screen-lg w-full overflow-hidden shadow-md">
            <table className="w-full table-auto">
              ...
              <tbody>
                {rejectedCollectors.map((collector, index) => (
                  <tr
                    key={collector._id}
...
export default RejectedCollectors;
